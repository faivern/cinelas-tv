...
namespace backend.Services.Tmdb
{
    public partial class TmdbService : ITmdbService
    {
        ...
        public async Task<string> GetShowSeasonAsync(int seriesId, int seasonNumber)
        {
            var url = $"https://api.themoviedb.org/3/tv/{seriesId}/season/{seasonNumber}?api_key={_apiKey}";
            return await FetchWithCacheAsync($"show_season_{seriesId}_{seasonNumber}", url, CacheDurations.Standard);
        }
        //----------------------------------------------------------------------------
        ...
        public async Task<string> DiscoverByGenreAsync(string mediaType, int genreId, int page, string sortBy = "popularity.desc")
        {
            ...
            // Unguarded rating sorts return obscure titles with a handful of votes.
            if (sortBy.StartsWith("vote_average", StringComparison.Ordinal))
            {
        ...
        // TMDB's collection payload lists parts without runtime, so each part is
        // enriched with it here (per-movie details are cached) — the TV shelf
        // shows the franchise's total watch time from a single request.
        public async Task<string> GetCollectionByIdAsync(int collectionId, string lang = "en-US")
        {
            var cacheKey = $"collection_by_id_enriched::{lang}::{collectionId}";
            if (_cache.TryGetValue(cacheKey, out string? cached) && cached is not null)
                return cached;

            var url = $"https://api.themoviedb.org/3/collection/{collectionId}?api_key={_apiKey}&language={lang}";
            var raw = await FetchWithCacheAsync($"collection_by_id::{lang}::{collectionId}", url, CacheDurations.StaticReference);

            var root = System.Text.Json.Nodes.JsonNode.Parse(raw);
            if (root?["parts"] is System.Text.Json.Nodes.JsonArray parts)
            {
                var runtimes = await Task.WhenAll(parts.Select(async part =>
                {
                    var id = part?["id"]?.GetValue<int>();
                    if (id is null) return (int?)null;
                    try
                    {
                        return (await GetMovieDetailsTypedAsync(id.Value))?.Runtime;
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "Runtime enrichment failed for movie {MovieId} in collection {CollectionId}", id, collectionId);
                        return null;
                    }
                }));
                ...
                {
                    if (runtimes[i] is int runtime && parts[i] is not null)
                        parts[i]!["runtime"] = runtime;
                }
            }

            var enriched = root?.ToJsonString() ?? raw;
            _cache.Set(cacheKey, enriched, new MemoryCacheEntryOptions
            {
                AbsoluteExpirationRelativeToNow = CacheDurations.StaticReference
            });
            return enriched;
        }
