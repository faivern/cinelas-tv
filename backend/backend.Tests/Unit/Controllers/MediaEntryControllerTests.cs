...
public class MediaEntryControllerTests : AuthenticatedControllerTestBase
{
    ...
    public async Task Update_ValidRatings_ReturnsOk()
    {
        ...
        var request = new UpdateMediaEntryRequest { RatingActing = 8.5, Status = WatchStatus.Favorites };
        var result = await _controller.Update(1, request);

        var ok = result.Should().BeOfType<OkObjectResult>().Subject;
        var updated = (MediaEntry)ok.Value!;
        updated.RatingActing.Should().Be(8.5);
        updated.Status.Should().Be(WatchStatus.Favorites);
    }
    ...
    public async Task Update_NonExistent_ReturnsNotFound()
    {
        ...
        var result = await _controller.Update(999, new UpdateMediaEntryRequest { Status = WatchStatus.Favorites });
