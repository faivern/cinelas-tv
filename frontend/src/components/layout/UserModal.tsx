...
import { Check, ChevronDown, List, LogOut, Palette, User } from "lucide-react";
...
export const UserModal = ({ userName, onLogout, onClose, show }: UserModalProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-2 border-t border-border/25 mt-1">
        ...
        >
          <div className="relative">
            <Listbox.Button className="relative w-full cursor-pointer rounded-lg bg-input border border-outline py-2 pl-3 pr-10 text-left text-text-h1     transition-all text-sm">
            ...
            <Transition
              ...
              leaveTo="opacity-0"
            >
              <Listbox.Options className="absolute z-(--z-dropdown) mt-1 w-full overflow-auto rounded-lg bg-action-primary border border-outline px-1 py-1 text-sm shadow-lg ">
      ...
      {/* Attribution */}
      <div className="border-t border-border/25 mt-1 px-4 pt-2">
        <p className="text-[10px] text-subtle/70 select-none">
          Metadata by TMDB · Streaming data by JustWatch
        </p>
      </div>
    </div>
  );
