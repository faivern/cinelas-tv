...
import { Check, Clock, Eye, Film, Plus, Tv, X } from "lucide-react";
...
const STATUS_DISPLAY: Record<WatchStatus, string> = isTvMode()
...
const WATCH_STATUSES: { value: WatchStatus; label: string }[] = (
  ["WantToWatch", "Watching", "Favorites"] as WatchStatus[]
).map((value) => ({ value, label: STATUS_DISPLAY[value] }));
...
const STATUS_COLORS: Record<WatchStatus, { border: string; bg: string; text: string }> = {
  ...
  Favorites: { border: "border-green-400", bg: "bg-green-500/20", text: "text-green-400" },
};
...
export default function AddToListModal({
  ...
  const handleAddClick = async () => {
    const needsRating =
      !isTvMode() && (status === "Watching" || status === "Favorites");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (hasSelectedLists && hasSelectedStatus) {
        ...
        toast.success(
          <span>Added to {listPart} and as <strong>{STATUS_DISPLAY[status]}</strong></span>,
        );
        ...
        toast.success(
          <span>Added as {' '}<strong>{STATUS_DISPLAY[status]}</strong></span>,
        );
      }
  ...
  const MediaIcon = media.mediaType === "movie" ? Film : Tv;
                    ...
                    <>
                      ...
                      <div className="p-4 border-b border-[var(--border)]">
                        ...
                        {isLoading ? (
                          ...
                          <div className="space-y-2 max-h-48 overflow-y-auto overscroll-contain p-3 -m-1">
                            {lists.map((list) => {
                      ...
                      <div className="flex items-center justify-end gap-3 p-4">
                        ...
                        >
                          {isSaving
                            ? "Adding..."
                            : status === "Watching" || status === "Favorites"
                              ? "Next"
                              : "Add"}
                        </button>
                      </div>
                    ...
                    <>
                      ...
                      <div className="p-4 border-b border-[var(--border)]">
                        ...
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-3 py-2 bg-input border border-outline rounded-lg text-white placeholder-gray-500     resize-none"
                        />
                      </div>
