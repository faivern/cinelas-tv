  ...
  return (
    <Listbox value={selectedCountry} onChange={onCountryChange}>
      <div className={`relative w-full sm:w-48 ${className}`}>
        <Listbox.Button className="relative w-full cursor-pointer rounded-lg bg-component-primary border border-outline py-2 pl-3 pr-10 text-left text-text-h1     transition-all text-sm">
        ...
        <Transition
          ...
          leaveTo="opacity-0"
        >
          <Listbox.Options className="absolute z-(--z-dropdown) mt-1 max-h-72 w-full overflow-hidden rounded-lg bg-component-primary border border-outline text-sm shadow-lg ">
            <div className="sticky top-0 bg-component-primary p-2 border-b border-outline">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-subtle h-3 w-3" />
                <input
                  type="text"
                  placeholder="Search countries..."
                  value={countrySearch}
                  onChange={(e) => setCountrySearch(e.target.value)}
                  className="w-full bg-input border border-outline rounded-md py-1.5 pl-8 pr-3 text-text-h1 placeholder:text-subtle   text-sm"
