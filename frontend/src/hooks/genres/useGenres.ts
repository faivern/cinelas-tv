...
import displayNames, { CANONICAL_GENRE_IDS } from "../../utils/genreMap";
    ...
    queryFn: async (): Promise<EnrichedGenre[]> => {
      ...
      for (const genre of movieGenres ?? []) {
        genreMap.set(genre.id, {
          id: genre.id,
          name: displayNames[genre.id] ?? genre.name,
          supportedMediaTypes: ["movie"] as MediaType[],
        });
      }

      // Process TV genres - add 'tv' to existing or create new entry.
      // TV-only split ids (10759/10765) fold into their movie counterpart
      // so "Action" and "Sci-Fi" each appear once.
      for (const genre of tvGenres ?? []) {
        const id = CANONICAL_GENRE_IDS[genre.id] ?? genre.id;
        ...
        if (existing) {
          ...
          genreMap.set(id, {
            id,
            name: displayNames[id] ?? genre.name,
