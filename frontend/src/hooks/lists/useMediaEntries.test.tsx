...
const mockEntries: MediaEntry[] = [
  makeMockEntry({ id: 1, tmdbId: 100, status: "Favorites" }),
  ...
  makeMockEntry({ id: 4, tmdbId: 400, status: "Favorites", title: "Another Favorites" }),
];
...
describe("useMediaEntries (lists/)", () => {
  ...
  describe("useMediaEntriesByStatus", () => {
    it("filters entries by Favorites status", async () => {
      vi.mocked(mediaEntriesApi.getUserEntries).mockResolvedValue(mockEntries);

      const { result } = renderHook(
        () => useMediaEntriesByStatus("Favorites"),
        { wrapper: createWrapper() },
      );

      await waitFor(() => expect(result.current.isSuccess).toBe(true));
      expect(result.current.data).toHaveLength(2);
      expect(result.current.data?.every((e) => e.status === "Favorites")).toBe(true);
    });
    ...
    it("returns empty array when no entries match", async () => {
      vi.mocked(mediaEntriesApi.getUserEntries).mockResolvedValue([
        makeMockEntry({ status: "Favorites" }),
      ]);
  ...
  describe("useUpdateMediaEntry", () => {
    it("calls update with id and data", async () => {
      const updated = makeMockEntry({ id: 1, status: "Favorites" });
      ...
      act(() => {
        result.current.mutate({ id: 1, data: { status: "Favorites" } });
      });

      await waitFor(() => expect(result.current.isSuccess).toBe(true));
      expect(mediaEntriesApi.update).toHaveBeenCalledWith(1, { status: "Favorites" });
    });
  });
  ...
  describe("useWatchStatusCounts", () => {
    it("returns counts per status", async () => {
      ...
      expect(result.current.Favorites).toBe(2);
