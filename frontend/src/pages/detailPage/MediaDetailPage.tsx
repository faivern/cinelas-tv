...
import SeasonEpisodes from "../../components/media/detail/SeasonEpisodes";
...
import TvMediaDetailPage from "./TvMediaDetailPage";
...
export default function MediaDetailPage({ mediaType }: { mediaType: MediaType }) {
  if (isTvMode()) return <TvMediaDetailPage mediaType={mediaType} />;
  return <WebMediaDetailPage mediaType={mediaType} />;
}

function WebMediaDetailPage({ mediaType: media_type }: { mediaType: MediaType }) {
  ...
  // The query hooks no-op (enabled: false) while id is missing/NaN.
  ...
  const { data: fetchedBackdropPaths = [] } = useMediaBackdrops(media_type, numericId);
  ...
  return (
    <main>
      ...
      {/* Full-page rotating backdrop behind all detail content. */}
      <div className="fixed inset-0 -z-10" aria-hidden="true">
        <RotatingBackdrop paths={backdropImagePaths} />
        <div className="absolute inset-0 bg-black/60" />
      </div>

      {/* Cinelas watermark — bottom-left corner. */}
      <div
        className="fixed bottom-6 left-6 z-(--z-raised) pointer-events-none opacity-60"
        aria-hidden="true"
      >
        <BrandLogo />
      </div>
      ...
      <div className="px-4 md:px-[8%] 4xl:px-[12%] mt-4">
        <div className="flex flex-col md:grid md:grid-cols-[1fr_auto] gap-6 md:gap-8 md:gap-x-12">
          ...
          {/* col 1, row 2 — episodes (TV only) */}
          {media_type === "tv" && details.seasons?.length ? (
            <div className="order-2 md:order-none min-w-0">
              <SeasonEpisodes seriesId={numericId} seasons={details.seasons} />
            </div>
          ) : null}

          {/* col 1, row 3 — watch providers */}
